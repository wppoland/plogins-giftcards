...
/**
 * PRO upgrade promotion, shown ONLY on the Gift Cards settings screen: a
...
{
    private const META   = 'giftcards_pro_banner_dismissed';
    private const ACTION = 'giftcards_dismiss_pro';
    ...
    {
        if ($this->data === null) {
            $file = GIFTCARDS_DIR . 'config/pro-upsell.php';
    ...
    public function enabled(): bool
    {
         ...
         */
        return (bool) apply_filters('giftcards/show_pro_cta', true) && $this->features() !== [];
    }

    private function url(): string
    {
        $default = (string) ($this->data()['url'] ?? 'https://plogins.com/plogins-giftcards-pro/pricing/');
        /**
         * Filters the URL the "Upgrade to PRO" buttons point at.
         *
         * @param string $url Default the Gift Cards PRO pricing page.
         */
        return (string) apply_filters('giftcards/pro_url', $default);
    }
    ...
    public function handleDismiss(): void
    {
        ...
        wp_safe_redirect(wp_get_referer() ?: admin_url('admin.php?page=giftcards-settings'));
        exit;
    }
    ...
    public function banner(): void
    {
        ...
        ?>
        <div class="giftcards-pro-banner" role="note">
            <span class="giftcards-pro-banner__tag">PRO</span>
            ...
            <a class="button button-primary giftcards-pro-banner__cta" href="<?php echo esc_url($this->url()); ?>" target="_blank" rel="noopener noreferrer">
                <?php esc_html_e('Upgrade to PRO', 'plogins-giftcards'); ?>
            </a>
        ...
        <aside class="giftcards-pro-aside" aria-labelledby="giftcards-pro-aside-h">
            ...
            <ul class="giftcards-pro-aside__list">
                <?php foreach ($features as $f) : ?>
                    <li>
                        <span class="giftcards-pro-aside__lock" aria-hidden="true"></span>
                        <span><?php echo esc_html($f['title']); ?></span>
                    </li>
                <?php endforeach; ?>
            </ul>
            <a class="button button-primary button-hero giftcards-pro-aside__cta" href="<?php echo esc_url($this->url()); ?>" target="_blank" rel="noopener noreferrer">
                <?php esc_html_e('Upgrade to PRO', 'plogins-giftcards'); ?>
            </a>
            <?php if ($price !== '') : ?>
                <p class="giftcards-pro-aside__price"><?php echo esc_html($price); ?> · <?php esc_html_e('one licence, every PRO feature', 'plogins-giftcards'); ?></p>
            <?php endif; ?>
        </aside>
        <?php
    }
    ...
    public function cards(): void
    {
        ...
        <section class="giftcards-pro-cards" aria-labelledby="giftcards-pro-cards-h">
            ...
            <div class="giftcards-pro-cards__grid">
                <?php foreach ($features as $f) : ?>
                    <article class="giftcards-pro-card">
                        <span class="giftcards-pro-card__badge">PRO</span>
                        <span class="giftcards-pro-card__lock" aria-hidden="true"></span>
                        <h3 class="giftcards-pro-card__title"><?php echo esc_html($f['title']); ?></h3>
                        <?php if ($f['desc'] !== '') : ?>
                            <p class="giftcards-pro-card__desc"><?php echo esc_html($f['desc']); ?></p>
                        <?php endif; ?>
                    </article>
                <?php endforeach; ?>
            </div>
