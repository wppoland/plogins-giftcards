...
final class Settings implements HasHooks
{
    ...
    private function help(string $id, string $text): void
    {
        printf(
            ...
            esc_attr($id),
            esc_attr__('More information', 'donkarto'),
            esc_html($text),
        );
    }
    ...
    public function addMenuPage(): void
    {
        add_submenu_page(
            'woocommerce',
            __('Donkarto: gift cards and store credit', 'donkarto'),
            __('Gift Cards', 'donkarto'),
            'manage_woocommerce',
            self::PAGE,
            [$this, 'renderPage'],
        );
    ...
    public function renderPage(): void
    {
        ...
        <div class="wrap giftcards-admin"
            ...
            <h1>
                ...
                <?php if ($enabled) : ?>
                    <span class="giftcards-admin__status giftcards-admin__status--on">
                        <?php esc_html_e('Active', 'donkarto'); ?>
                    </span>
                <?php else : ?>
                    <span class="giftcards-admin__status giftcards-admin__status--off">
                        <?php esc_html_e('Disabled', 'donkarto'); ?>
                    </span>
                <?php endif; ?>
            </h1>
            ...
            <div class="giftcards-admin__intro">
                ...
                <div>
                    <h2><?php esc_html_e('Sell gift cards in three steps', 'donkarto'); ?></h2>
                    <p>
                        <?php esc_html_e('1. Flag a product as a gift card (product editor > General tab > Gift card). 2. A buyer purchases it. 3. A unique code is emailed to the recipient and can be redeemed at checkout for a discount. Tune the wording below.', 'donkarto'); ?>
                    </p>
                </div>
            </div>
            ...
            <form method="post" action="options.php">
                ...
                <div class="giftcards-admin__card">
                    <h2><?php esc_html_e('General', 'donkarto'); ?></h2>
                    <p class="giftcards-admin__card-hint">
                        <?php esc_html_e('Core behaviour: whether gift cards are sold and how codes look.', 'donkarto'); ?>
                    </p>
                            ...
                            <tr>
                                <th scope="row">
                                    <span class="giftcards-admin__label">
                                        <?php esc_html_e('Enable gift cards', 'donkarto'); ?>
                                        <?php $this->help('gc-help-enabled', __('Master switch. When off, no codes are issued, the checkout redeem field is hidden, and existing balances cannot be spent. Turn this off to pause the feature without losing data.', 'donkarto')); ?>
                                    </span>
                                </th>
                                <td>
                                    <label for="giftcards_enabled">
                                        ...
                                        />
                                        <?php esc_html_e('Generate and email a code when a gift-card product is purchased, and accept codes at checkout.', 'donkarto'); ?>
                                    </label>
                                </td>
                            </tr>
                            <tr>
                                <th scope="row">
                                    <span class="giftcards-admin__label">
                                        <label for="giftcards_code_prefix"><?php esc_html_e('Code prefix', 'donkarto'); ?></label>
                                        <?php $this->help('gc-help-prefix', __('A short tag added to the front of every code so customers and support can recognise it at a glance. Example: "GIFT-" produces GIFT-AB12CD34. Leave blank for codes with no prefix.', 'donkarto')); ?>
                                    </span>
                                </th>
                                <td>
                                    <input
                                        ...
                                        maxlength="12"
                                    />
                                    <p class="description"><?php esc_html_e('Optional. Prepended to every generated code, e.g. "GIFT-". Up to 12 characters.', 'donkarto'); ?></p>
                                </td>
                            </tr>
                            <tr>
                                <th scope="row">
                                    <span class="giftcards-admin__label">
                                        <label for="giftcards_fee_label"><?php esc_html_e('Checkout discount label', 'donkarto'); ?></label>
                                        <?php $this->help('gc-help-fee', __('The text shown on the discount line at checkout when a code is applied (e.g. "Gift card (GIFT-AB12CD34)"). Use the {code} token to include the applied code.', 'donkarto')); ?>
                                    </span>
                                </th>
                                <td>
                                    <input
                                        ...
                                        class="regular-text"
                                    />
                                    <p class="description"><?php esc_html_e('Shown on the discount line at checkout. Use {code} for the applied code.', 'donkarto'); ?></p>
                                </td>
                            </tr>
                            <tr>
                                <th scope="row">
                                    <span class="giftcards-admin__label">
                                        <?php esc_html_e('Show codes on order', 'donkarto'); ?>
                                        <?php $this->help('gc-help-show', __('When a customer buys a gift card for themselves, this lists the issued code(s) right on the order-confirmation page and in their order emails, so they get the code instantly without waiting for the separate recipient email.', 'donkarto')); ?>
                                    </span>
                                </th>
                                <td>
                                    <label for="giftcards_show_codes_on_order">
                                        ...
                                        />
                                        <?php esc_html_e('List the issued gift-card codes on the buyer\'s order-confirmation page and in their order emails.', 'donkarto'); ?>
                                    </label>
                                </td>
                            </tr>
                        </tbody>
                ...
                <div class="giftcards-admin__card">
                    <h2><?php esc_html_e('Recipient email', 'donkarto'); ?></h2>
                    <p class="giftcards-admin__card-hint">
                        <?php esc_html_e('The email sent to whoever receives the gift card. Click a token to insert it where your cursor is.', 'donkarto'); ?>
                    </p>

                    <div class="giftcards-tokens" role="group" aria-label="<?php esc_attr_e('Insert a template token', 'donkarto'); ?>">
                        <button type="button" class="giftcards-token" data-token="{code}"
                            data-inserted="<?php esc_attr_e('inserted', 'donkarto'); ?>"
                            data-copied="<?php esc_attr_e('copied', 'donkarto'); ?>">{code}</button>
                        <button type="button" class="giftcards-token" data-token="{amount}"
                            data-inserted="<?php esc_attr_e('inserted', 'donkarto'); ?>"
                            data-copied="<?php esc_attr_e('copied', 'donkarto'); ?>">{amount}</button>
                    </div>
                    ...
                    <table class="form-table" role="presentation">
                        <tbody>
                            <tr>
                                <th scope="row">
                                    <label for="giftcards_email_subject"><?php esc_html_e('Subject', 'donkarto'); ?></label>
                                </th>
                                <td>
                                    <input
                                        type="text"
                            ...
                            <tr>
                                <th scope="row">
                                    <label for="giftcards_email_body"><?php esc_html_e('Body', 'donkarto'); ?></label>
                                </th>
                                <td>
                                    <textarea
                                    ...
                                    <p class="description"><?php esc_html_e('{code} becomes the gift-card code; {amount} becomes the formatted value.', 'donkarto'); ?></p>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                    ...
                    <div class="giftcards-preview" aria-hidden="true">
                        <span class="giftcards-preview__label"><?php esc_html_e('Live preview', 'donkarto'); ?></span>
